(() => {
  ...
  function getStats() {
    ...
    return {
      ...
      fileMB: parseInt(byteLength / (1024 * 1024))
    };
  }
  ...
  function readFile(data) {
    return new Promise((resolve) => {
      ...
      reader.onload = () => {
        ...
        function getDataSection(arrBuff) {
  ...
  // src_client/ifc-reader/ifc-reader.ts
  var IFCReader = class extends HTMLElement {
    constructor() {
      super(...arguments);
      this.memory = `${performance.memory.usedJSHeapSize / Math.pow(1e3, 2)} MB`;
      this.memoryAfter = "tba";
      this.time = 0;
      this.text = "";
    }
    render() {
      ...
      <div class="flex flex-col border-1 border-gray-400 p-2 m-2">
        ...
        <button
          @click=${() => {
        this.memoryAfter = `${performance.memory.usedJSHeapSize / Math.pow(1e3, 2)} MB`;
        this.render();
      }}
